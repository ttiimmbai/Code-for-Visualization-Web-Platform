...
function getOrientationMarkerPositions(element) {
  ...
  coords = {
    x: enabledElement.image.width / 2,
    y: enabledElement.image.height - 10
  };
...
var _freehandanno = __webpack_require__(/*! ./imageTools/freehandanno.js */ "./imageTools/freehandanno.js");

Object.defineProperty(exports, 'freehandanno', {
    enumerable: true,
    get: function get() {
        return _freehandanno.freehandanno;
    }
});
...
var _ctr = __webpack_require__(/*! ./imageTools/ctr.js */ "./imageTools/ctr.js");
...
Object.defineProperty(exports, 'ctrTouch', {
  enumerable: true,
  get: function get() {
    return _ctr.ctrTouch;
  }
});
...
//更改颜色工具颜色
var defaultColor = 'yellow',//'red',
    activeColor = 'red',//'greenyellow',
...
exports.fillOutsideFree = fillOutsideFree;
...
//遍历判断前后不一样参数的下标
function beforeIndex(options,maxindex){
    for(var i=0;i<options.length;i++){
        if(maxindex - i<1){
            if(options[maxindex+options.length-i-1].x<options[maxindex].x){
                return maxindex+options.length-i-1;
            }
        }else{
            if(options[maxindex-i].x<options[maxindex].x){
                return maxindex-i;
            }
        }
    }
...
//遍历判断后边不一样参数的下标
function afterIndex(options,maxindex){
    for(var i=0;i<options.length;i++){
        if(maxindex + i>(options.length-1)){
            if(options[maxindex+i-options.length].x<options[maxindex].x){
                return maxindex+i-options.length;
            }
        }else{
            if(options[maxindex+i].x<options[maxindex].x){
                return maxindex+i;
...
//判断是否为顺时针
function isClockwise (options){
    var maxindex = 0;
    var maxX = 0;
    options.map(function(item,index,arr){
        if(item.x>maxX){
            maxX = item.x;
            maxindex = index;
        }
    });
    var beIndex = beforeIndex(options,maxindex);
    var afIndex = afterIndex(options,maxindex);
    var flag = (options[maxindex].x-options[beIndex].x)*(options[afIndex].y-options[maxindex].y)-(options[maxindex].y-options[beIndex].y)*(options[afIndex].x-options[maxindex].x);
    if(flag<0){
        return true;
    }else{
        return false;
 ...
 * 自由遮盖的效果
 * */
function fillOutsideFree(context, element, start, points, options) {
    var coordSystem = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : 'pixel';
    ...
    var left = Math.min(start.x, start.x);
    var top = Math.min(start.y, start.y);
...
/***/ }),
    ...
    /***/ (function(module, exports, __webpack_require__) {
        ...
        exports.freehandanno = undefined;
...
// Freehand tool libraries
         ...
         */
        function pointNearHandle(element, data, coords) {
            var config = freehandanno.getConfiguration();
...
// On each click, if it intersects with a current point, end drawing loop
         ...
         * from existing handles.
         *
         * @param {Object} eventData - data object associated with an event.freehand
         */
        function startDrawing(eventData) {
            var element = eventData.element;
         ...
         *
         * @param {Object} eventData - Data object associated with an event.
         * @param {Object} dataHandles - Data object associated with the tool.freehand
         */
        function addPointPencilMode(eventData, dataHandles) {
            var config = freehandanno.getConfiguration();
        ...
        function addPoint(eventData) {
            ...
            // Get the toolData from the last-drawn polygon freehand
            var config = freehandanno.getConfiguration();
            var data = toolData.data[config.currentTool];
         ...
         *
         * @param {Object} eventData - data object associated with an event.
         * @param {Object} handleNearby - the handle nearest to the mouse cursor.freehand
         */
        function endDrawing(eventData, handleNearby) {
            var toolData = (0, _toolState.getToolState)(eventData.element, toolType);
            ...
            //(0, _drawing.fillOutsideFree)(context, eventData.element, data.handles[0], data.handles, options);
...
// /////// END ACTIVE TOOL ///////
        ...
        function mouseMoveCallback(e) {
            ...
            // Force onImageRendered freehand
            _externalModules2.default.cornerstone.updateImage(eventData.element);
        }
         ...
         * Returns true if the proposed location of a new handle is invalid (in polygon mode).
         *
         * @param {Object} data - data object associated with the tool. freehand
         * @return {Boolean}
         */
        function checkHandlesPolygonMode(data) {
            var config = freehandanno.getConfiguration();
         ...
         *
         * @event
         * @param {Object} e - The event. freehand
         */
        function mouseDragCallback(e) {
            var eventData = e.detail;
            var toolData = (0, _toolState.getToolState)(eventData.element, toolType);
         ...
         * Event handler called by mouseDownPassive which modifies a tool's textBox.
         *
         * @param {Object} element - The element associated with the event.freehand
         * @param {Object} nearby - Object containing information about a nearby handle.
         */
        function modifyTextBox(element, nearby) {
            var config = freehandanno.getConfiguration();
         ...
         * Gets the current mouse location and stores it in the configuration object.
         *
         * @param {Object} eventData - The data assoicated with the event.freehand
         */
        function getMouseLocation(eventData) {
            // Set the mouseLocation handle
            var config = freehandanno.getConfiguration();
...
// /////// BEGIN IMAGE RENDERING ///////
        ...
        function onImageRendered(e) {
            ...
            // If we have no toolData for this element, return immediately as there is nothing to do freehand
            var toolData = (0, _toolState.getToolState)(e.currentTarget, toolType);
            ...
            var _loop = function _loop(i) {
                ...
                (0, _drawing.draw)(context, function (context) {
                    ...
                    // Only render text if polygon ROI has been completed and freehand 'shiftKey' mode was not used:
                    if (data.polyBoundingBox && !data.textBox.freehandanno) {
                        // If the textbox has not been moved by the user, it should be displayed on the right-most
                        // Side of the tool.
                        (0, _drawing.fillOutsideFree)(context, eventData.element, data.handles[0], data.handles, options1);
                        if (!data.textBox.hasMoved) {
                            // Find the rightmost side of the polyBoundingBox at its vertical center, and place the textbox here
                            // Note that this calculates it in image coordinates
                            data.textBox.x = data.polyBoundingBox.left + data.polyBoundingBox.width;
...
// /////// END IMAGE RENDERING ///////
         ...
         * complete when changing tool mode.
         *
         * @param  {Object} element The element the ROI is associated with.freehand
         */
        function closeToolIfDrawing(element) {
            var config = freehandanno.getConfiguration();
            if (config.currentTool >= 0) {
...
// Module/private exports
        ...
        exports.freehandanno = freehandanno;
/***/ }),
/***/ "./imageTools/ctr.js":
/*!******************************!*\
  !*** ./imageTools/length.js ***!
...
/***/ (function(module, exports, __webpack_require__) {
        ...
        exports.ctrTouch = exports.ctr = undefined;
        ...
        function getHandle(x, y, index, extraAttributes={}) {
...
// /////// BEGIN ACTIVE TOOL ///////
        function createNewMeasurement(mouseEventData) {
          // Create the measurement data for this tool with the end handle activated
          var { x, y } = mouseEventData.currentPoints.image;
          // Create the measurement data for this tool with the end handle activated
          var measurementData = {
            toolType,
            isCreating: true,
            ...
            handles: {
              ...
              //短线右边左端点
              perpendicularStart3: getHandle(x, y, 6, { locked: true }),
              //短线右边右端点
              perpendicularEnd3: getHandle(x, y, 7),
              //长线右边左端点
              perpendicularStart4: getHandle(x, y, 8, { locked: true }),
              //长线右边右端点
              perpendicularEnd4: getHandle(x, y, 9),
              textBox: getHandle(x - 50, y - 80, null, {
                highlight: false,
                movesIndependently: false,
                drawnIndependently: true,
                allowedOutsideImage: true,
...
//更新垂线句柄
        function updatePerpendicularLineHandles (eventData, data) {
          ...
          if (!data.handles.perpendicularStart2.locked) return;
          if (!data.handles.perpendicularStart3.locked) return;
          if (!data.handles.perpendicularStart4.locked) return;

          let startX, startY, endX, endY,startX2, startY2, endX2, endY2,startX3, startY3, endX3, endY3,startX4, startY4, endX4, endY4;

          var { start, end } = data.handles;
          if (start.x === end.x && start.y === end.y) {
            ...
            var mid = {
              x: (start.x + end.x) / 2,
              y: (start.y + end.y) / 2,
              x2: (start.x+(end.x - start.x)/3),
              y2: (start.y + (end.y - start.y)/3),
              x3: (start.x+(end.x - start.x)/3),
              y3: (start.y + (end.y - start.y)/3),
              x4: (start.x + end.x) / 2,
              y4: (start.y + end.y) / 2,
            ...
            var perpendicularLineLength2 = length / 3;

            startX = mid.x + (perpendicularLineLength / 2) * vectorY;
            startY = mid.y - (perpendicularLineLength / 2) * vectorX;
            endX = mid.x - 0.2*vectorY;
            endY = mid.y + 0.2*vectorX;
            startX2 = mid.x2 + (perpendicularLineLength2 / 2) * vectorY;
            startY2 = mid.y2 - (perpendicularLineLength2 / 2) * vectorX;
            //此处解决了半边线突然闪到端点的问题
            //endX2 = mid.x2;
            //endY2 = mid.y2;
            endX2 = mid.x2-0.2*vectorY;
            endY2 = mid.y2+0.2*vectorX;
            //startX3 = mid.x3;
            //startY3 = mid.y3;
            startX3 = mid.x3+0.2*vectorY;
            startY3 = mid.y3-0.2*vectorX;
            endX3 = mid.x3 -(perpendicularLineLength2 / 2) * vectorY;
            endY3 = mid.y3 + (perpendicularLineLength2 / 2) * vectorX;
            startX4 = mid.x4+0.2*vectorY;
            startY4 = mid.y4-0.2*vectorX;
            endX4 = mid.x4 -(perpendicularLineLength / 2) * vectorY;
            endY4 = mid.y4 + (perpendicularLineLength / 2) * vectorX;
          }
          ...
          data.handles.perpendicularStart2.x = startX2;
          data.handles.perpendicularStart2.y = startY2;
          data.handles.perpendicularEnd2.x = endX2;
          data.handles.perpendicularEnd2.y = endY2;

          data.handles.perpendicularStart3.x = startX3;
          data.handles.perpendicularStart3.y = startY3;
          data.handles.perpendicularEnd3.x = endX3;
          data.handles.perpendicularEnd3.y = endY3;

          data.handles.perpendicularStart4.x = startX4;
          data.handles.perpendicularStart4.y = startY4;
          data.handles.perpendicularEnd4.x = endX4;
          data.handles.perpendicularEnd4.y = endY4;
...
// 画垂线
        function drawPerpendicularLine (context, element, data, color, lineWidth) {
          // Draw perpendicular line
          var { perpendicularStart, perpendicularEnd, perpendicularStart2, perpendicularEnd2 , perpendicularStart3, perpendicularEnd3, perpendicularStart4, perpendicularEnd4} = data.handles;
          var perpendicularStartCanvas = cornerstone.pixelToCanvas(element, perpendicularStart);
          var perpendicularEndCanvas = cornerstone.pixelToCanvas(element, perpendicularEnd);
          var perpendicularStartCanvas2 = cornerstone.pixelToCanvas(element, perpendicularStart2);
          var perpendicularEndCanvas2 = cornerstone.pixelToCanvas(element, perpendicularEnd2);
          var perpendicularStartCanvas3 = cornerstone.pixelToCanvas(element, perpendicularStart3);
          var perpendicularEndCanvas3 = cornerstone.pixelToCanvas(element, perpendicularEnd3);
          var perpendicularStartCanvas4 = cornerstone.pixelToCanvas(element, perpendicularStart4);
          var perpendicularEndCanvas4 = cornerstone.pixelToCanvas(element, perpendicularEnd4);
          ...
          context.strokeStyle = color;
          context.lineWidth = lineWidth;
          context.moveTo(perpendicularStartCanvas2.x, perpendicularStartCanvas2.y);
          context.lineTo(perpendicularEndCanvas2.x, perpendicularEndCanvas2.y);
          context.stroke();

          context.beginPath();
          context.strokeStyle = color;
          context.lineWidth = lineWidth;
          context.moveTo(perpendicularStartCanvas3.x, perpendicularStartCanvas3.y);
          context.lineTo(perpendicularEndCanvas3.x, perpendicularEndCanvas3.y);
          context.stroke();

          context.beginPath();
          context.strokeStyle = color;
          context.lineWidth = lineWidth;
          context.moveTo(perpendicularStartCanvas4.x, perpendicularStartCanvas4.y);
          context.lineTo(perpendicularEndCanvas4.x, perpendicularEndCanvas4.y);
          context.stroke();
        }
//以下是mouseDownCallback准备的

        function perpendicularBothFixedLeft (eventData, data) {
          var { distance } = _externalModules2.default.cornerstoneMath.point;
          var { start, end, perpendicularStart, perpendicularEnd,perpendicularStart2, perpendicularEnd2 ,perpendicularStart3, perpendicularEnd3,perpendicularStart4, perpendicularEnd4} = data.handles;
          var { image } = eventData.currentPoints;
          ...
          var perpendicularLine3 = {
            start: {
              x: perpendicularStart3.x,
              y: perpendicularStart3.y
            },
            end: {
              x: perpendicularEnd3.x,
              y: perpendicularEnd3.y
            }
          };
          var perpendicularLine4 = {
            start: {
              x: perpendicularStart4.x,
              y: perpendicularStart4.y
            },
            end: {
              x: perpendicularEnd4.x,
              y: perpendicularEnd4.y
            }
          };

          //var intersection = _externalModules2.default.cornerstoneMath.lineSegment.intersectLine(longLine, perpendicularLine);
          var intersection = perpendicularEnd;
          ...
          var newLineLength = distance(end, image);

          if (newLineLength <= distanceToLineP2) {
            return false;
          }
          //var intersection2 = _externalModules2.default.cornerstoneMath.lineSegment.intersectLine(longLine, perpendicularLine2);
          var intersection2 = perpendicularEnd2;
          var distanceFromPerpendicularP12 = distance(perpendicularStart2, intersection2);
          var distanceFromPerpendicularP22 = distance(perpendicularEnd2, intersection2);

          var distanceToLineP22 = distance(end, intersection2);
          var newLineLength2 = distance(end, image);

          if (newLineLength <= distanceToLineP2) {
            return false;
          }
          if (newLineLength2 <= distanceToLineP22) {
            return false;
          }

          //var intersection3 = _externalModules2.default.cornerstoneMath.lineSegment.intersectLine(longLine, perpendicularLine3);
          var intersection3 = perpendicularStart3;

          var distanceFromPerpendicularP13 = distance(perpendicularStart3, intersection3);
          var distanceFromPerpendicularP23 = distance(perpendicularEnd3, intersection3);

          var distanceToLineP23 = distance(end, intersection3);
          var newLineLength3 = distance(end, image);

          if (newLineLength <= distanceToLineP2) {
            return false;
          }
          if (newLineLength3 <= distanceToLineP23) {
            return false;
          }

          var intersection4 = perpendicularStart4;

          var distanceFromPerpendicularP14 = distance(perpendicularStart4, intersection4);
          var distanceFromPerpendicularP24 = distance(perpendicularEnd4, intersection4);

          var distanceToLineP24 = distance(end, intersection4);
          var newLineLength4 = distance(end, image);

          if (newLineLength <= distanceToLineP2) {
            return false;
          }
          if (newLineLength4 <= distanceToLineP24) {
            return false;
          }

          var dx = (end.x - image.x) / newLineLength;
          var dy = (end.y - image.y) / newLineLength;

          var k = distanceToLineP2 / newLineLength;

          var dx2 = (end.x - image.x) / newLineLength2;
          var dy2 = (end.y - image.y) / newLineLength2;

          var k2 = distanceToLineP22 / newLineLength2;

          var dx3 = (end.x - image.x) / newLineLength3;
          var dy3 = (end.y - image.y) / newLineLength3;

          var k3 = distanceToLineP23 / newLineLength3;

          var dx4 = (end.x - image.x) / newLineLength4;
          var dy4 = (end.y - image.y) / newLineLength4;

          var k4 = distanceToLineP24 / newLineLength4;
          ...
          var newIntersection2 = {
            x: end.x + ((image.x - end.x) * k2),
            y: end.y + ((image.y - end.y) * k2)
          };

          var newIntersection3 = {
            x: end.x + ((image.x - end.x) * k3),
            y: end.y + ((image.y - end.y) * k3)
          };

          var newIntersection4 = {
            x: end.x + ((image.x - end.x) * k4),
            y: end.y + ((image.y - end.y) * k4)
          };
          ...
          perpendicularStart2.x = newIntersection2.x - distanceFromPerpendicularP12 * dy2;
          perpendicularStart2.y = newIntersection2.y + distanceFromPerpendicularP12 * dx2;

          perpendicularEnd2.x = newIntersection2.x + distanceFromPerpendicularP22 * dy2;
          perpendicularEnd2.y = newIntersection2.y - distanceFromPerpendicularP22 * dx2;


          perpendicularStart3.x = newIntersection3.x - distanceFromPerpendicularP13 * dy3;
          perpendicularStart3.y = newIntersection3.y + distanceFromPerpendicularP13 * dx3;

          perpendicularEnd3.x = newIntersection3.x + distanceFromPerpendicularP23 * dy3;
          perpendicularEnd3.y = newIntersection3.y - distanceFromPerpendicularP23 * dx3;

          perpendicularStart4.x = newIntersection4.x - distanceFromPerpendicularP14 * dy4;
          perpendicularStart4.y = newIntersection4.y + distanceFromPerpendicularP14 * dx4;

          perpendicularEnd4.x = newIntersection4.x + distanceFromPerpendicularP24 * dy4;
          perpendicularEnd4.y = newIntersection4.y - distanceFromPerpendicularP24 * dx4;
          return true;
        }
        ...
        function perpendicularBothFixedRight (eventData, data) {
          ...
          var newLineLength = distance(start, image);
          ...
          var distanceToLineP22 = distance(start, intersection2);
          var newLineLength2 = distance(start, image);
          ...
          var distanceToLineP23 = distance(start, intersection3);
          var newLineLength3 = distance(start, image);
          ...
          var distanceToLineP24 = distance(start, intersection4);
          var newLineLength4 = distance(start, image);
          ...
          var dx = (start.x - image.x) / newLineLength;
          var dy = (start.y - image.y) / newLineLength;
          ...
          var dx2 = (start.x - image.x) / newLineLength2;
          var dy2 = (start.y - image.y) / newLineLength2;

          var k2 = distanceToLineP22 / newLineLength2;

          var newIntersection2 = {
            x: start.x + ((image.x - start.x) * k2),
            y: start.y + ((image.y - start.y) * k2)
          };

          var dx3 = (start.x - image.x) / newLineLength3;
          var dy3 = (start.y - image.y) / newLineLength3;

          var k3 = distanceToLineP23 / newLineLength3;

          var newIntersection3 = {
            x: start.x + ((image.x - start.x) * k3),
            y: start.y + ((image.y - start.y) * k3)
          };

          var dx4 = (start.x - image.x) / newLineLength4;
          var dy4 = (start.y - image.y) / newLineLength4;

          var k4 = distanceToLineP24 / newLineLength4;

          var newIntersection4 = {
            x: start.x + ((image.x - start.x) * k4),
            y: start.y + ((image.y - start.y) * k4)
          };
...
//对于下边左边线左端点的操作
        function perpendicularLeftFixedPoint (eventData, data) {
          var { distance } = _externalModules2.default.cornerstoneMath.point;
          var { start, end, perpendicularStart, perpendicularEnd } = data.handles;
          ...
          var distanceFromFixed = _externalModules2.default.cornerstoneMath.lineSegment.distanceToPoint(data.handles, fixedPoint);
          var distanceFromMoved = _externalModules2.default.cornerstoneMath.lineSegment.distanceToPoint(data.handles, movedPoint);
...
//对于左边上边那根线左端点的操作
        function perpendicularLeftFixedPoint2 (eventData, data) {
          var { distance } = _externalModules2.default.cornerstoneMath.point;
          var { start, end, perpendicularStart, perpendicularEnd, perpendicularStart2, perpendicularEnd2 , perpendicularStart3, perpendicularEnd3} = data.handles;

          var fudgeFactor = 1;

          var fixedPoint = perpendicularEnd2;
          var movedPoint = eventData.currentPoints.image;
...
//对于上边右边那根线左端点的操作
        function perpendicularLeftFixedPoint3 (eventData, data) {
          var { distance } = _externalModules2.default.cornerstoneMath.point;
          var { start, end, perpendicularStart, perpendicularEnd, perpendicularStart2, perpendicularEnd2 , perpendicularStart3, perpendicularEnd3} = data.handles;

          var fudgeFactor = 1;

          var fixedPoint = perpendicularEnd3;
          var movedPoint = perpendicularStart3;
          ...
          perpendicularStart3.x = movedPoint.x;
          perpendicularStart3.y = movedPoint.y;
          perpendicularEnd3.x = movedPoint.x - total * dy;
          perpendicularEnd3.y = movedPoint.y + total * dx;
          perpendicularEnd3.locked = false;
          perpendicularStart3.locked = false;
          ...
          if (!intersection) {
            if (distance(movedPoint, start) > distance(movedPoint, end)) {
              perpendicularStart3.x = adjustedLineP2.x + distanceFromMoved * dy;
              perpendicularStart3.y = adjustedLineP2.y - distanceFromMoved * dx;
              perpendicularEnd3.x = perpendicularStart3.x - total * dy;
              perpendicularEnd3.y = perpendicularStart3.y + total * dx;
            } else {
              perpendicularStart3.x = adjustedLineP1.x + distanceFromMoved * dy;
              perpendicularStart3.y = adjustedLineP1.y - distanceFromMoved * dx;
              perpendicularEnd3.x = perpendicularStart3.x - total * dy;
              perpendicularEnd3.y = perpendicularStart3.y + total * dx;
            }
          }
...
//对于下边右边线左端点的操作
        function perpendicularLeftFixedPoint4 (eventData, data) {
          var { distance } = _externalModules2.default.cornerstoneMath.point;
          var { start, end, perpendicularStart, perpendicularEnd, perpendicularStart2, perpendicularEnd2 , perpendicularStart3, perpendicularEnd3, perpendicularStart4, perpendicularEnd4} = data.handles;

          var fudgeFactor = 1;

          var fixedPoint = perpendicularEnd4;
          var movedPoint = perpendicularStart4;
          ...
          perpendicularStart4.x = movedPoint.x;
          perpendicularStart4.y = movedPoint.y;
          perpendicularEnd4.x = movedPoint.x - total * dy;
          perpendicularEnd4.y = movedPoint.y + total * dx;
          perpendicularEnd4.locked = false;
          perpendicularStart4.locked = false;
          ...
          if (!intersection) {
            if (distance(movedPoint, start) > distance(movedPoint, end)) {
              perpendicularStart4.x = adjustedLineP2.x + distanceFromMoved * dy;
              perpendicularStart4.y = adjustedLineP2.y - distanceFromMoved * dx;
              perpendicularEnd4.x = perpendicularStart4.x - total * dy;
              perpendicularEnd4.y = perpendicularStart4.y + total * dx;
            } else {
              perpendicularStart4.x = adjustedLineP1.x + distanceFromMoved * dy;
              perpendicularStart4.y = adjustedLineP1.y - distanceFromMoved * dx;
              perpendicularEnd4.x = perpendicularStart4.x - total * dy;
              perpendicularEnd4.y = perpendicularStart4.y + total * dx;
            }
          }
...
//对于下边左线右端点的操作
        ...
        function perpendicularRightFixedPoint (eventData, data) {
          ...
          var movedPoint = perpendicularEnd;
...
//对于上边左边那根线右端点的操作
        ...
        function perpendicularRightFixedPoint2 (eventData, data) {
          var { distance } = _externalModules2.default.cornerstoneMath.point;
          var { start, end, perpendicularStart, perpendicularEnd, perpendicularStart2, perpendicularEnd2 } = data.handles;

          var fudgeFactor = 1;

          var fixedPoint = perpendicularStart2;
          var movedPoint = perpendicularEnd2;
...
//对于上边右边那根线右端点的操作
        ...
        function perpendicularRightFixedPoint3 (eventData, data) {
          var { distance } = _externalModules2.default.cornerstoneMath.point;
          var { start, end, perpendicularStart, perpendicularEnd, perpendicularStart2, perpendicularEnd2 , perpendicularStart3, perpendicularEnd3} = data.handles;

          var fudgeFactor = 1;

          var fixedPoint = perpendicularStart3;
          var movedPoint = eventData.currentPoints.image;;
          ...
          perpendicularStart3.x = movedPoint.x + total * dy;
          perpendicularStart3.y = movedPoint.y - total * dx;
          perpendicularEnd3.x = movedPoint.x;
          perpendicularEnd3.y = movedPoint.y;
          perpendicularEnd3.locked = false;
          perpendicularStart3.locked = false;
          ...
          if (!intersection) {
            if (distance(movedPoint, start) > distance(movedPoint, end)) {
              perpendicularEnd3.x = adjustedLineP2.x - distanceFromMoved * dy;
              perpendicularEnd3.y = adjustedLineP2.y + distanceFromMoved * dx;
              perpendicularStart3.x = perpendicularEnd3.x + total * dy;
              perpendicularStart3.y = perpendicularEnd3.y - total * dx;
            } else {
              perpendicularEnd3.x = adjustedLineP1.x - distanceFromMoved * dy;
              perpendicularEnd3.y = adjustedLineP1.y + distanceFromMoved * dx;
              perpendicularStart3.x = perpendicularEnd3.x + total * dy;
              perpendicularStart3.y = perpendicularEnd3.y - total * dx;
            }
          }
...
//对于下边右边那根线右端点的操作
        ...
        function perpendicularRightFixedPoint4 (eventData, data) {
          var { distance } = _externalModules2.default.cornerstoneMath.point;
          var { start, end, perpendicularStart, perpendicularEnd, perpendicularStart2, perpendicularEnd2 , perpendicularStart3, perpendicularEnd3, perpendicularStart4, perpendicularEnd4} = data.handles;

          var fudgeFactor = 1;

          var fixedPoint = perpendicularStart4;
          var movedPoint = eventData.currentPoints.image;;
          ...
          perpendicularStart4.x = movedPoint.x + total * dy;
          perpendicularStart4.y = movedPoint.y - total * dx;
          perpendicularEnd4.x = movedPoint.x;
          perpendicularEnd4.y = movedPoint.y;
          perpendicularEnd4.locked = false;
          perpendicularStart4.locked = false;
          ...
          if (!intersection) {
            if (distance(movedPoint, start) > distance(movedPoint, end)) {
              perpendicularEnd4.x = adjustedLineP2.x - distanceFromMoved * dy;
              perpendicularEnd4.y = adjustedLineP2.y + distanceFromMoved * dx;
              perpendicularStart4.x = perpendicularEnd4.x + total * dy;
              perpendicularStart4.y = perpendicularEnd4.y - total * dx;
            } else {
              perpendicularEnd4.x = adjustedLineP1.x - distanceFromMoved * dy;
              perpendicularEnd4.y = adjustedLineP1.y + distanceFromMoved * dx;
              perpendicularStart4.x = perpendicularEnd4.x + total * dy;
              perpendicularStart4.y = perpendicularEnd4.y - total * dx;
            }
          }
...
// 设置手柄位置(开始，结束，垂线开始，垂线)
        function setHandlesPosition (handle, eventData, data) {
          ...
          if (handle.index === 0) {
            ...
            intersection = _externalModules2.default.cornerstoneMath.lineSegment.intersectLine(longLine, perpendicularLine);
            if (!intersection) {
              perpendicularLine.end = {
              ...
              d1 = _externalModules2.default.cornerstoneMath.point.distance(intersection, data.handles.start);
              d2 = _externalModules2.default.cornerstoneMath.point.distance(intersection, data.handles.end);
            ...
            //intersection = _externalModules2.default.cornerstoneMath.lineSegment.intersectLine(longLine, perpendicularLine);
            intersection = perpendicularLine.end;
            if (!intersection) {
              perpendicularLine.end = {
              ...
              d1 = _externalModules2.default.cornerstoneMath.point.distance(data.handles.perpendicularEnd2, data.handles.start);
              d2 = _externalModules2.default.cornerstoneMath.point.distance(data.handles.perpendicularEnd2, data.handles.end);
            ...
            if (!outOfBounds) {
              movedPoint = perpendicularLeftFixedPoint2(eventData, data);
              if (!movedPoint) {
                eventData.currentPoints.image.x = data.handles.perpendicularStart2.x;
                eventData.currentPoints.image.y = data.handles.perpendicularStart2.y;
              }
            }
            ...
            if (!outOfBounds) {
              movedPoint = perpendicularRightFixedPoint2(eventData, data);

              if (!movedPoint) {
                eventData.currentPoints.image.x = data.handles.perpendicularEnd2.x;
                eventData.currentPoints.image.y = data.handles.perpendicularEnd2.y;
              }
            }
            ...
            perpendicularLine.start = {
              x: data.handles.perpendicularEnd3.x,
              y: data.handles.perpendicularEnd3.y
            };
            perpendicularLine.end = {
              x: data.handles.perpendicularStart3.x,
              y: data.handles.perpendicularStart3.y
            };
            ...
            if (!outOfBounds) {
              movedPoint = perpendicularLeftFixedPoint3(eventData, data);

              if (!movedPoint) {
                eventData.currentPoints.image.x = data.handles.perpendicularStart3.x;
                eventData.currentPoints.image.y = data.handles.perpendicularStart3.y;
              }
            }
            ...
            //intersection = _externalModules2.default.cornerstoneMath.lineSegment.intersectLine(longLine, perpendicularLine);
            intersection = perpendicularLine.start;
            if (!intersection) {
              perpendicularLine.end = {
                x: data.handles.perpendicularEnd3.x,
                y: data.handles.perpendicularEnd3.y
              ...
              d1 = _externalModules2.default.cornerstoneMath.point.distance( 	data.handles.perpendicularStart3, data.handles.start);
              d2 = _externalModules2.default.cornerstoneMath.point.distance(	data.handles.perpendicularStart3, data.handles.end);
            ...
            if (!outOfBounds) {
              movedPoint = perpendicularRightFixedPoint3(eventData, data);

              if (!movedPoint) {
                eventData.currentPoints.image.x = data.handles.perpendicularEnd3.x;
                eventData.currentPoints.image.y = data.handles.perpendicularEnd3.y;
              }
            }
          }
            ...
            perpendicularLine.start = {
              x: data.handles.perpendicularEnd4.x,
              y: data.handles.perpendicularEnd4.y
            };
            perpendicularLine.end = {
              x: data.handles.perpendicularStart4.x,
              y: data.handles.perpendicularStart4.y
            };
            ...
            if (!outOfBounds) {
              movedPoint = perpendicularLeftFixedPoint4(eventData, data);

              if (!movedPoint) {
                eventData.currentPoints.image.x = data.handles.perpendicularStart4.x;
                eventData.currentPoints.image.y = data.handles.perpendicularStart4.y;
              }
            }
            ...
            if (!intersection) {
              ...
              d1 = _externalModules2.default.cornerstoneMath.point.distance( 	data.handles.perpendicularStart4, data.handles.start);
              d2 = _externalModules2.default.cornerstoneMath.point.distance(	data.handles.perpendicularStart4, data.handles.end);
            ...
            if (!outOfBounds) {
              movedPoint = perpendicularRightFixedPoint4(eventData, data);

              if (!movedPoint) {
                eventData.currentPoints.image.x = data.handles.perpendicularEnd4.x;
                eventData.currentPoints.image.y = data.handles.perpendicularEnd4.y;
              }
            }
          }
        }

        var handleActivator = (element, handles, canvasPoint, distanceThreshold=6) => {
          var getHandle = cornerstoneTools.getHandleNearImagePoint;
          var nearbyHandle = getHandle(element, handles, canvasPoint, distanceThreshold);
...
// 当鼠标离开时，mouseMoveCallback用于隐藏句柄
        function mouseMoveCallback(event) {
          ...
          for (let i = 0; i < toolData.data.length; i++) {
            ...
            var nearToolAndInactive = pointNearTool(element, data, coords) && !data.active;
            var notNearToolAndActive = !pointNearTool(element, data, coords) && data.active;
        ...
        function invertHandles(eventData, measurementData, handle) {
          var { rowPixelSpacing, columnPixelSpacing } = eventData.image;
          var { handles } = measurementData;
          var { start, end, perpendicularStart, perpendicularEnd } = handles;
...
// 清除所有工具句柄的双向工具选择
        ...
        function moveHandle (mouseEventData, toolType, data, handle, doneMovingCallback, preventHandleOutsideImage) {
          ...
          var mouseDragCallback = event => {
            ...
            var measurementModifiedHandler = () => {
          ...
          var imageRenderedHandler = () => {
            var newImage = cornerstone.getImage(element);
...
// mouseDownCallback用于限制垂线的行为
        function mouseDownCallback (event) {
          var eventData = event.detail;
          var data;
          var element = eventData.element;
          var $element = $(element);
          var options = cornerstoneTools.getToolOptions(toolType, element);
          ...
          // 添加事件侦听器，以在激活度量时清除所选状态
          var activateEventKey = 'ViewerMeasurementsActivated';
          ...
          if (eventData.which === 1) {
            var imageNeedsUpdate = clearBidirectionalSelection(event);
            if (imageNeedsUpdate) {
              cornerstone.updateImage(element);
            }
          }
          ...
          var coords = eventData.startPoints.canvas;
          var toolData = cornerstoneTools.getToolState(event.currentTarget, toolType);
          ...
          for (let i = 0; i < toolData.data.length; i++) {
            data = toolData.data[i];
            var handleParams = [element, data.handles, coords, 6];
            var handle = cornerstoneTools.getHandleNearImagePoint(...handleParams);
          ...
          var getDoneMovingCallback = handles => () => {
          ...
          for (let i = 0; i < toolData.data.length; i++) {
            ...
            if (pointNearTool(element, data, coords)) {
              ...
              var allHandlesParams = [event, data, toolData, toolType, opt, doneMovingCallback];
...
// 计算给定测量数据的最长和最短直径
        function calculateLongestAndShortestDiameters (eventData, measurementData) {
          var { rowPixelSpacing, columnPixelSpacing } = eventData.image;
          var { start, end, perpendicularStart, perpendicularEnd, perpendicularStart2, perpendicularEnd2 , perpendicularStart3, perpendicularEnd3, perpendicularStart4, perpendicularEnd4} = measurementData.handles;
          ...
          var wx2 = (perpendicularStart2.x - perpendicularEnd2.x) * (columnPixelSpacing || 1);
          var wy2 = (perpendicularStart2.y - perpendicularEnd2.y) * (rowPixelSpacing || 1);
          var width2 = Math.sqrt(wx2 * wx2 + wy2 * wy2);
          if (!width2) {
            width2 = 0;
          }

          var wx3 = (perpendicularStart3.x - perpendicularEnd3.x) * (columnPixelSpacing || 1);
          var wy3 = (perpendicularStart3.y - perpendicularEnd3.y) * (rowPixelSpacing || 1);
          var width3 = Math.sqrt(wx3 * wx3 + wy3 * wy3);
          if (!width3) {
            width3 = 0;
          }

          var wx4 = (perpendicularStart4.x - perpendicularEnd4.x) * (columnPixelSpacing || 1);
          var wy4 = (perpendicularStart4.y - perpendicularEnd4.y) * (rowPixelSpacing || 1);
          var width4 = Math.sqrt(wx4 * wx4 + wy4 * wy4);
          if (!width4) {
            width4 = 0;
          }

          //var width4 = width2+width3;
          ...
          measurementData.shortestDiameter = (width+width4).toFixed(1);
          measurementData.shortestDiameter2 = (width2+width3).toFixed(1);
          //measurementData.shortestDiameter3 = width3.toFixed(1);
          measurementData.ctr = Math.round((width2+width3)/(width+width4)*100)/100;
        }

        const pointNearPerpendicular = (element, handles, coords) => {
          const lineSegment = {
            start: _externalModules2.default.cornerstone.pixelToCanvas(element, handles.perpendicularStart),
            end: _externalModules2.default.cornerstone.pixelToCanvas(element, handles.perpendicularEnd)
          };

          const distanceToPoint = _externalModules2.default.cornerstoneMath.lineSegment.distanceToPoint(lineSegment, coords);

          return (distanceToPoint < 6);
        };
...
//以上试新加的
        function pointNearTool(element, data, coords) {
          var { handles } = data;
          var lineSegment = {
            start: _externalModules2.default.cornerstone.pixelToCanvas(element, handles.start),
            end: _externalModules2.default.cornerstone.pixelToCanvas(element, handles.end)
          };
...
// /////// BEGIN IMAGE RENDERING ///////
        function onImageRendered(e) {
          ...
          var imagePlane = _externalModules2.default.cornerstone.metaData.get('imagePlaneModule', eventData.image.imageId);
          var rowPixelSpacing;
          var colPixelSpacing;
          var element = eventData.element;
          ...
          var color;
          var lineWidth = _toolStyle2.default.getToolWidth();
          var config = ctr.getConfiguration();
          ...
          for (let i = 0; i < toolData.data.length; i++) {
            ...
            var { start, end, perpendicularStart, perpendicularEnd, textBox } = data.handles;
            ...
            var activeColor = _toolColors2.default.getActiveColor();
            if (data.active) {
              color = activeColor;
            } else {
            ...
            // Draw the line
            var { pixelToCanvas } = _externalModules2.default.cornerstone;
            var handleStartCanvas = pixelToCanvas(element, start);
            var handleEndCanvas = pixelToCanvas(element, end);
            var handlePerpendicularStartCanvas = pixelToCanvas(element, perpendicularStart);
            var handlePerpendicularEndCanvas = pixelToCanvas(element, perpendicularEnd);
            var canvasTextLocation = pixelToCanvas(element, textBox);
            ...
            // Draw the handles
            var handlesColor = color;
            //drawHandles(context, eventData, data.handles, handlesColor, { drawHandlesIfActive: true });
            (0, _drawHandles2.default)(context, eventData, data.handles, handlesColor, { drawHandlesIfActive: true });
            ...
            if (data) {
              ...
              var lengthText =  data.shortestDiameter + suffix;
              var widthText =  (data.shortestDiameter2) + '/';
              let textLines = [  widthText+lengthText,'ctr:'+data.ctr];
              ...
              var boundingBox = cornerstoneTools.drawTextBox(
                  context,
                  textLines,
                  canvasTextLocation.x+10,
                  canvasTextLocation.y+100,
                  color,
                  config.textBox
              );
              ...
              //repositionTextBox(eventData, data, config.textBox);
              ...
              // 检查垂线是否有一定长度(起点和终点不相等)
              // Note: this check is needed to prevent NaN value on the intersection result
              // 注意:需要进行此检查以防止交集结果上的NaN值
              var { distance } = _externalModules2.default.cornerstoneMath.point;
              var lineHasLength = distance(perpendicularLine.start, perpendicularLine.end) > 0;
              ...
              if (lineHasLength) {
                ...
                //由于线具有长度，因此将其定义为线之间的交点
                var { intersectLine } = _externalModules2.default.cornerstoneMath.lineSegment;
...
// Module exports
        var ctr = (0, _mouseButtonTool2.default)({
          createNewMeasurement: createNewMeasurement,
          onImageRendered: onImageRendered,
          pointNearTool: pointNearTool,
          toolType: toolType,
        ...
        var ctrTouch = (0, _touchTool2.default)({
          createNewMeasurement: createNewMeasurement,
          onImageRendered: onImageRendered,
          pointNearTool: pointNearTool,
          toolType: toolType,
